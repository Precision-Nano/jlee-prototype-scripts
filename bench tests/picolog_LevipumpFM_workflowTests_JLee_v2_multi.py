...
Description: Process Levitronix pump and Levitronix FM setup in wetlab for water-ethanol tests run by Paul Shen. (ENG-000680).
Test plan G, particularly for multiple tests baked into one file. Compiled results in "resultsArr2".
Last updated: Apr 5, 22
...
for filename in os.listdir(pathName):
    if nameStr in filename and '.csv' in filename and int(test) <= int(testEnd):
        testNum = int(test)
        ...
        for i in range(round(1245/dt),rawPump.shape[0]): #usually start at 0.
