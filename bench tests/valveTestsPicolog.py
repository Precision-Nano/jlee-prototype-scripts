...
Final results in "results_all" array.
...
for filename in os.listdir(pathName):
    if nameStr in filename and 'psiA.csv' in filename and int(test) <= int(testEnd):
            ...
            for i in range (1,rawVC.shape[0]):         
                ...
                if i1<rawVC.shape[0]:
                    if rawVC[i1] == 0 and rawVC[i1] != rawVC[i1-1]:
                        ...
                        for j in range(i1,i1+round((onT+3)/dt),1):
                            ...
                            # Find end conditions.
                            if rawV1C[j] != rawV1C[j-1] and rawV1C[j] != v1state and j != i1:
                                vcn = np.append(vcn,j)
        ...
        [res_valveArr, cyc_count] = valve_times(rawV1C, rawV1S1, rawV1S2, onT, dt)
